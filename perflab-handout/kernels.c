...
team_t team = {
    ...
    "Heather Seaman",     /* First member full name */
    "hseaman@hmc.edu",  /* First member email address */

    "Tessa Barton",                   /* Second member full name (leave blank if none) */
...
static pixel avg_perf(int dim, int i, int j, pixel *src) 
{
    int ii, jj;
    pixel_sum sum;
    pixel current_pixel;
...
void smooth(int dim, pixel *src, pixel *dst) 
{
    ...
    for (i = 0; i < dim; i++) {
        for (j = 0; j < dim; j++) {
            dst[dimi+ j] = avg_perf(dim, i, j, src);
        }
        dimi += dim;
    }
}
