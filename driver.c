...
static void create(int dim)
{
    ...
    /* Align the images to BSIZE byte boundaries.  Assumes power of 2. */
    orig = (pixel *)(((unsigned long)data + BSIZE - 1) & ~(BSIZE - 1));
    result = orig + dim*dim;
...
static int check_smooth(int dim) {
    ...
    /* Get rid of compiler warnings */
    right.blue = right.green = right.red = 0;
    wrong.blue = wrong.green = wrong.red = 0;
